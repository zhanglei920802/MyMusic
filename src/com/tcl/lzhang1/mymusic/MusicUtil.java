...
import com.tcl.lzhang1.mymusic.R.string;
...
public class MusicUtil {
	 ...
	 * @throws SDCardUnMoutedException
	 */
	public static List<SongModel> scanMusic(Context context)
			throws SDCardUnMoutedException {
	...
	public static void scanFile(File file) {
		...
		if (!file.isHidden()) {
			...
			if (file.isFile()) {
				...
				SongModel model = getMusicInfo(file.getAbsolutePath());
	...
	public static SongModel getMusicInfo(File musicFile) {
		...
		try {
			...
			Cursor cursor = contentResolver.query(
					...
					"_data=?", new String[] { musicFile.getAbsolutePath() },
					null);
			cursor.moveToFirst();
			...
			Log.d(LOG_TAG,
					String.format(
							"scaned music file[%s],album name[%s],song name[%s],singer name[%s]",
							model.getFile(), model.getAblumName(),
							model.getSingerName(), model.getSingerName()));
	 ...
	 * @return
	 */
	public static int getPercent(int curMin, int curSec, int totalMin,
			int totalSec) {

		return (int) (((curMin * 60 + curSec) / (Float
				.parseFloat((totalMin * 60 + totalSec) + ""))) * 100);
	}
